		...
		const int* cls_ids)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// 	int target_index = cls_ids[i + collected_id[j] * C];  
			// 	if (target_index >= 0 && target_index < C) {  // Bounds check
			// 		tmp[ch] = features[i + collected_id[j] * C];  
			// 	}
			// }
			...
			// {
			// 	// const float c = collected_colors[ch * BLOCK_SIZE + j];
			// 	// cls_idx = cls_ids[collected_id[j] * C + ch];
			// 	const float c = colors[collected_id[j] *  + ch];
			// 	// Update last color (to be used in the next iteration)
			// 	accum_rec[ch] = last_alpha * last_color[ch] + (1.f - last_alpha) * accum_rec[ch];
			// 	last_color[ch] = c;

			// 	const float dL_dchannel = dL_dpixel[ch];C
			// 	dL_dalpha += (c - accum_rec[ch]) * dL_dchannel;
			// 	// Update the gradients w.r.t. color of the Gaussian.
			// 	// Atomic, since this pixel is just one of potentially
			// 	// many that were affected by this Gaussian.
